...
icacls "C:\Users\$UserName\.ssh" /inheritance:r /grant:r "${UserName}:(OI)(CI)F" "SYSTEM:(OI)(CI)F" | Out-Null
...
icacls $Sandbox /inheritance:r /grant:r "${UserName}:(OI)(CI)F" "SYSTEM:(OI)(CI)F" | Out-Null
...
$new | Set-Content $sshdConf -Encoding UTF8
Restart-Service sshd
Write-Host "    sshd_config 已改, 服务已重启"
